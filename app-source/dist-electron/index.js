...
function pgyOverviewTextWidth(a, e) {
  let t = 0;
  for (const n of Array.from(String(a ?? ""))) t += n.charCodeAt(0) > 255 ? e : e * 0.55;
  return t;
}
...
function pgyOverviewFitText(a, e, t) {
  ...
  for (const i of Array.from(n)) {
    if (pgyOverviewTextWidth(s + i, e) + e * 1.65 > t) break;
    s += i;
  }
  return s + "...";
}
...
function pgyBuildBloggerOverviewData(a) {
  ...
  const r = pgyOverviewList(pgyOverviewPick(t, ["contentTags"]));
  const c = pgyOverviewList(pgyOverviewPick(t, ["featureTags", "personalTags", "tags"]));
  const u = [];
  for (const D of [...r, ...c]) u.includes(D) || u.push(D);
  ...
  const genderLabel = pgyOverviewLabel(pgyOverviewPick(t, ["gender"]));
  return {
    ...
    healthLevelState: level == null || level < 0 ? "" : (level === 0 ? "abnormal" : "healthy"),
    travelAreaText: pgyOverviewList(pgyOverviewPick(t, ["travelAreaList", "travelAreas"])).join("、") || "-",
    categoryTags: u.slice(0, 5),
...
function pgyBloggerOverviewSvg(a) {
  ...
  const nickname = pgyOverviewFitText(e.nickname ?? "-", 24, 236);
  let iconX = 252 + Math.ceil(pgyOverviewTextWidth(nickname, 24)) + 14;
  let profileIcons = "";
  if (e.genderText === "女" || e.genderText === "男") {
    const fill = e.genderText === "女" ? "#f8a0bb" : "#8ab6ee";
    profileIcons += `<circle cx="${iconX + 12}" cy="186" r="12" fill="${fill}"/><text x="${iconX + 5}" y="193" font-size="16" fill="#fff">${e.genderText === "女" ? "♀" : "♂"}</text>`;
    iconX += 34;
  }
  if (e.healthLevelState === "healthy" || e.healthLevelState === "abnormal") {
    const hx = iconX, hy = 174, fill = e.healthLevelState === "healthy" ? "#02B940" : "#FF7D03";
    const shield = `M${hx + 11} ${hy}L${hx + 22} ${hy + 5}L${hx + 22} ${hy + 14}Q${hx + 22} ${hy + 21} ${hx + 11} ${hy + 26}Q${hx} ${hy + 21} ${hx} ${hy + 14}L${hx} ${hy + 5}Z`;
    const mark = e.healthLevelState === "healthy"
      ? `<polyline points="${hx + 6},${hy + 13} ${hx + 9.5},${hy + 17} ${hx + 16},${hy + 8}" fill="none" stroke="#fff" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>`
      : `<line x1="${hx + 11}" y1="${hy + 6}" x2="${hx + 11}" y2="${hy + 15}" stroke="#fff" stroke-width="2" stroke-linecap="round"/><circle cx="${hx + 11}" cy="${hy + 19}" r="1.4" fill="#fff"/>`;
    profileIcons += `<path d="${shield}" fill="${fill}"/>${mark}`;
  }
  const hasTravel = e.travelAreaText && e.travelAreaText !== "-";
  const travelRow = hasTravel
    ? `<path d="M252 306 L272 294 L262 310 Z" fill="#b9bec7"/><text x="280" y="308" font-size="18" fill="#595959">${t(pgyOverviewEllipsize(e.travelAreaText, 12))}</text>`
    : "";
  const tags = Array.isArray(e.categoryTags) ? e.categoryTags.slice(0, 5) : [];
  const tagY = hasTravel ? 336 : 303;
  let tagX = 126, tagSvg = "";
  for (const n of tags) {
    const label = pgyOverviewFitText(n, 18, 120);
    const w = Math.ceil(pgyOverviewTextWidth(label, 18)) + 20;
    if (tagX + w > 590) break;
...
<rect x="96" y="120" width="520" height="500" rx="14" fill="#fff"/><circle cx="178" cy="216" r="52" fill="#e8edf4"/><text x="164" y="229" font-size="34" font-weight="700" fill="#6d87a8">${o}</text>${r}<text x="252" y="194" font-size="24" font-weight="600" fill="#262626">${t(nickname)}</text>${profileIcons}
<text x="252" y="236" font-size="19" fill="#999">小红书号：</text><text x="354" y="236" font-size="20" fill="#5273b4">${t(e.redId)}</text><text x="252" y="276" font-size="18" fill="#999">● ${t(pgyOverviewEllipsize(e.location, 10))}</text><rect x="420" y="262" width="12" height="14" fill="none" stroke="#777" stroke-width="2"/><line x1="424" y1="265" x2="424" y2="273" stroke="#777"/><line x1="428" y1="265" x2="428" y2="273" stroke="#777"/><text x="440" y="276" font-size="18" fill="#5273b4">${t(pgyOverviewEllipsize(e.mcn, 8))}</text>${travelRow}${tagSvg}
