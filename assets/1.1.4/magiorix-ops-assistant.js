(function () {
  ...
  const MAX_EXPORT_ROWS_PER_TASK = 2000;
  const PACE = {
  ...
  function bindBridge() {
    ...
    task.onItemResult((event) => {
      ...
      if (event.status === "success") {
        item.success += 1;
        const row = normalizeExportRow(event.data);
        if (row && item.rows.length < MAX_EXPORT_ROWS_PER_TASK) item.rows.push(row);
    ...
    task.onComplete((event) => {
      ...
      const record = {
        ...
        rows: item.rows.slice(0, MAX_EXPORT_ROWS_PER_TASK),
        exportTruncated: item.rows.length >= MAX_EXPORT_ROWS_PER_TASK,
      };
      state.history = [record, ...state.history.filter((old) => old.id !== record.id)].slice(0, MAX_HISTORY_ITEMS);
  ...
  async function downloadHistoryTask(taskId) {
    ...
    if (rows.length === 0) {
      log("warn", "历史任务没有可下载数据；旧版本记录只保存摘要，需重新采集后下载");
      return;
    }
    try {
      const result = await window.bridge.scraper.export.toExcel({
        fileName: safeFileName(record.fileName, `${record.id}.xlsx`),
        data: rows,
      });
      if (result?.success) {
        ...
        ${state.history.length ? state.history.map((item) => `
          <div class="moa-history">
            ...
            <div class="moa-row" style="margin-top:7px;margin-bottom:0">
              <button class="moa-btn" data-download-history="${escapeHtml(item.id)}" ${Array.isArray(item.rows) && item.rows.length ? "" : "disabled"}>下载文档</button>
              ${item.exportTruncated ? '<span class="moa-sub">仅保存前 2000 条</span>' : ""}
            </div>
          </div>
