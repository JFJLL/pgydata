...
外部授权仅允许关闭上述空平台交易号 High，不启动新的审查轮。统一入账函数现对 `transactionId` 字符串化并 `trim()`，空值在任何事务/余额更新前拒绝；查询和写库均使用规范值；已支付订单仅在传入值与数据库交易号完全一致时作为幂等重放。

新增五项回归覆盖微信/支付宝空交易号、两渠道已支付订单不同交易号重放、同渠道同交易号跨订单复用。初始定向测试 `1/5 pass`；修复后 `5/5 pass`。按固定反向验证临时删除中央空值判断后，两条空交易号测试 `0/2 pass`，微信实际余额 `150 !== 100`、支付宝 HTTP `200 !== 400`；恢复后 `2/2 pass`。全套后端测试 `29/29 pass`、skipped 0、todo 0。
