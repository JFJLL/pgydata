...
test("serialize path-or-label 优先级 path > label > 原节点", async () => {
  ...
  const onlyPath = tree.data[1];
  const onlyLabel = tree.data[2];
  assert.deepEqual(
    schema.serialize({
...
test("serialize path-trim 去掉首尾空格", async () => {
  ...
  assert.equal(schema.serialize({ payloadField: "location", value: guangdong }), "中国 > 广东省");
  assert.deepEqual(
...
test("loadOptions：live 成功 → source=live + 规范化节点 + LKG 已保存", async (t) => {
  ...
  const snapshot = await lkgStore.load("kolTagsV2");
  assert.ok(snapshot);
  assert.equal(snapshot.version, SCHEMA_VERSION);
  assert.equal(snapshot.provider, "kolTagsV2");
...
test("loadOptions：consumeBehavior / areas live 成功并带正确 payloadField", async (t) => {
  ...
  assert.equal(areasResult.nodes[0].children[0].fullPath, "中国 > 广东省");
});

test("loadOptions：request 抛错 + LKG 快照 → source=lkg + warning", async (t) => {
  const baseDir = await tmpDir(t);
  const lkgStore = createJsonLkgStore({ baseDir });
  await lkgStore.save("kolTagsV2", {
...
test("loadOptions：手写合法 LKG fixture 文件可直接回退", async (t) => {
  ...
  const baseDir = await tmpDir(t);
  await fs.writeFile(
    path.join(baseDir, "lkg-kolTagsV2.json"),
...
test("地域 loadOptions 保留原始 path，path-trim 对规范化节点生效", async () => {
  ...
  const guangdong = collectNodes(nodes).find((node) => node.label === "广东省");
  assert.ok(guangdong, "广东省 节点必须存在");
  assert.equal(guangdong.path, " 中国 > 广东省 ", "规范化节点必须保留原始 path");
  assert.equal(
    schema.serialize({ payloadField: "location", value: guangdong }),
    "中国 > 广东省",
    "path-trim 必须对规范化节点生效",
  );
});

test("audience20 配置链路：loadOptions -> top20CrowdsLabel -> top20-transform", async () => {
  ...
  assert.deepEqual(
    ...
    ["18-24岁 18-24岁-女性", "25-29岁", "30-39岁 30-39岁-女性"],
    "top20-transform 必须消费规范化节点 label 并应用官网变换",
  );
});
