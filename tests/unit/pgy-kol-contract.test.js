...
test("version stays at 1.3.2 with approved password registration when SMS is disabled", () => {
  const desktop = JSON.parse(read("app-source/package.json"));
  assert.equal(desktop.version, "1.3.2");
  assert.equal(desktop.assetsVersion, "1.3.2");
});
