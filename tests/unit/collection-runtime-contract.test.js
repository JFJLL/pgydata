...
test("source package, asset version, and backend package stay aligned at 1.1.6", () => {
  const desktop = JSON.parse(read("app-source/package.json"));
  const backend = JSON.parse(read("red-magic-api/package.json"));
  const assets = JSON.parse(read("assets/1.1.6/version.json"));
  assert.equal(desktop.version, "1.1.6");
  assert.equal(desktop.assetsVersion, "1.1.6");
  assert.equal(backend.version, "1.1.6");
  assert.equal(assets.version, "1.1.6");
});
