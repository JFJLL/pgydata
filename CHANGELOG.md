...
- 新增：桌面端一键上传诊断信息与诊断包功能（设置页 →「诊断与反馈」），支持选择最近关联任务与输入问题描述，一键收集系统环境、软件版本、最近运行日志、任务安全摘要、结构化执行轨迹与网络异常。
- 新增：统一脱敏模块（`DiagnosticRedactor`），对 Token、satoken、Cookie、密码、手机号、邮箱、本地用户名路径与 URL query 实行全量严格脱敏，业务采集内容与凭据绝不上报。
- 新增：任务执行诊断轨迹（Diagnostic Trace），覆盖任务启动、状态变更、授权、请求、解析、重试及失败各阶段，支持非阻塞异步写入与 14 天/50MB 自动清理。
- 新增：服务端诊断管理（`diagnostic_reports` 数据库 Migration v6、创建报告元数据接口、私有 ZIP 上传接口、30 天自动留存过期与注销清理）。
- 新增：管理后台「诊断中心」视图（支持报告编号/任务/手机号搜索、平台与状态筛选、详细环境与任务时间线查看、管理员私有 ZIP 安全下载）。
- 新增：离线与上传失败回退支持（断网或上传异常时可将脱敏诊断包直接保存导出至本地磁盘）。
- 升级：桌面端版本、前端资源版本与服务端版本同步升级至 `1.4.5`。
- 发布状态：当前仅为本地 Candidate；尚未修改 `latest.json`、未部署生产环境。

## 1.4.4 (Release)

- 优化：修复蒲公英博主主页「合作30天」与「合作90天」外溢进店中位数字段采集、弹窗选择与 Excel 导出完整呈现。
- 优化：恢复任务采集完成后停留在完成界面并支持选择导出路径与查看历史任务。
- 修复：内置绘图渲染程序采用逐张渲染与严格超时控制，完整支持全部图表生成与文字渲染；主进程新增 macOS `python3` 探测支持。
- 新增：为 macOS 平台补充完整的 `electron-builder` DMG 构建配置、一键构建脚本（`scripts/build-magiorix-mac-dmg.sh`）与详细构建说明（`README_MAC.md`）。
- 升级：桌面端版本、前端资源版本与服务端版本同步升级至 `1.4.4`。
- 本地安装包：`desktop-versions/windows/1.4.4/magiorix-desktop-1.4.4-windows.exe`（SHA256：`44555cf6825c9c54fdbea69ad46d5f12466e9425883247518402394bce3b10b6`，大小：106,670,015 字节）。
- 本地资源包：`red-magic-api/public/assets/desktop/1.4.4/assets.zip`（SHA256：`0a904055eb306c0052f704c74a26418d323a269df74616d33cf58dff69e23161`，大小：2,590,884 字节）。
- Mac 纯代码包：`D:\\download\\pic-vec\\magiorix-1.4.4-mac-code.zip`（SHA256：`7da14ea6bc29667b7efb129066a0e0c4f7b1b776ef4d2a0c80820aa293bbd1e1`，大小：5,813,523 字节）。
- 发布状态：已晋升发布（`latest.json` 已更新，服务端接口已就绪，北京 ECS 服务已同步重启）。
